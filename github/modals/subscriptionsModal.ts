...
export async function pullDetailsModal({ data, modify, read, persistence, http, slashcommandcontext, uikitcontext }: { data?, modify: IModify, read: IRead, persistence: IPersistence, http: IHttp ,slashcommandcontext?: SlashCommandContext, uikitcontext?: UIKitInteractionContext }): Promise<IUIKitModalViewParam> {
    ...
    if (user?.id) {
        ...
        block.addSectionBlock({
            text: { text: `#${pullData?.title}`, type: TextObjectType.PLAINTEXT },
        ...
        block.addContextBlock({ elements: [ block.newPlainTextObject(`Author: ${pullData?.user?.login} | `),block.newPlainTextObject(`State : ${pullData?.state} | `),block.newPlainTextObject(`Mergeable : ${pullData?.mergeable}`) ]});
        ...
        for (let file of pullRequestFiles) {
            ...
            let deletions = file["deletions"];
                block.addSectionBlock({
                    text: { text: `${index} ${fileName}`, type: TextObjectType.PLAINTEXT },
                    accessory: block.newButtonElement({
                        actionId: ModalsEnum.VIEW_FILE_ACTION,
                        text: {
                ...
                block.addContextBlock({ elements: [ block.newPlainTextObject(`Status: ${status} | `),block.newPlainTextObject(`Additions : ${addition} | `),block.newPlainTextObject(`Deletions : ${deletions}`) ]});
