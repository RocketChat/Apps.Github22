    ...
    public async run(
        ...
        try {
            ...
            switch (actionId) {
                ...
                case ModalsEnum.SHARE_GIST_ACTION: {
                    ...
                    if (value != undefined && access_token != undefined) {
                        ...
                        block.addContextBlock({
                            elements: [
                                ...
                                block.newPlainTextObject(
                                    new Date(val.updated_at).toISOString()
                                ),
                            ],
                        });

                        const files = new Map<string, IGistFile>(
                            Object.entries(val.files)
                        );
                ...
                case ModalsEnum.SHARE_PROFILE_EXEC: {
                    ...
                    const userProfile = await getBasicUserInfo(
                        this.http,
                        accessToken.token
                    );

                    const idRecord = new RocketChatAssociationRecord(
                        RocketChatAssociationModel.MISC,
                        "ProfileShareParam"
                    );

                    const profileData = await this.read
                        .getPersistenceReader()
                        .readByAssociation(idRecord);
                    ...
                    if (profileShareParams.includes("contributionGraph")) {
                        block.addImageBlock({
                            imageUrl: `https://activity-graph.herokuapp.com/graph?username=${userProfile.username}&bg_color=ffffff&color=708090&line=24292e&point=24292e`,
                            altText: "Github Contribution Graph",
                        });
                    }
                ...
                case ModalsEnum.DELETE_SUBSCRIPTION_ACTION: {
                    ...
                    const modal = await deleteSubsciptionsModal({
                        modify: this.modify,
                        read: this.read,
                        persistence: this.persistence,
                        http: this.http,
